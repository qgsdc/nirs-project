...
これまでの判定結果に基づき、解析に使用できないデータを除去して、最終的な解析用データセットを作成します。このステップにより、統計解析の信頼性を担保します。

#### 除外基準
1. **セッション単位の除外**: 
   Step 5 の数値指標判定で「Bad」とラベル付けされたセッション（計 19 件）を削除。
2. **被験者単位の除外**: 
   有効なセッション数が極端に少ない被験者を解析対象から除外。
   - **対象**: iwamoto 氏
   - **理由**: 12 セッション中 3 セッションのみが有効（生存率 25%）。個人差を推定するためのデータが不十分であり、グループ全体の統計結果を歪めるリスクがあるため。

#### 実行方法
MATLABのコマンドウィンドウで以下を実行してください。
